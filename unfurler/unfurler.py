...
def get(url, connect=5, read=5):
    uaheader = {'User-Agent': 'RapidUnfurl/1.1'}
    getspan = trace.get_current_span()
    getspan.set_attribute("unfurler.get.url", url)
    try:
        x = None
        x = requests.get(url, timeout=(connect, read), headers=uaheader)
        getspan.set_attribute("unfurler.get.status_code", x.status_code)
        ...
        getspan.set_attribute("unfurler.get.exception", "ConnectTimeout")
        getspan.set_attribute("unfurler.get.status_code", x.status_code)
        getspan.set_attribute("unfurler.get.error_type", str(e))
        ...
        x.status_code = 408
        x._content = ex.encode()
        getspan.set_attribute("unfurler.get.exception", "ReadTimeout")
        getspan.set_attribute("unfurler.get.status_code", x.status_code)
        getspan.set_attribute("unfurler.get.error_type", str(e))
        ...
        x.status_code = 418
        x._content = ex.encode()
        getspan.set_attribute("unfurler.get.exception", "OtherException")
        getspan.set_attribute("unfurler.get.status_code", x.status_code)
        getspan.set_attribute("unfurler.get.error_type", str(e))
...
def unfurl(url, connect_timeout=2, read_timeout=2, refresh_oembed_provider_list=False):
    ...
    span.add_event("performing the GET request to the URL specified")

    # This new-span declaration should happen where the `get` method is called, not inside the `get` method itself
    # Starting the new span inside the `get` method can cause duplicate spans because other methods in here also use the `get` method.
    with tracer.start_as_current_span("try-requests-get"):
        r = get(url, connect=connect_timeout, read=read_timeout)

    span.add_event("Validating that the GET worked")
    if not r.ok:
        data = extend_dict(data, {"status_code": str(r.status_code), "exception": str(r.error_type)})
        return data

    span.add_event("pushing data into pyquery")
    try:
        r_pq = pq(r.text)
        span.set_attribute("unfurler.pyquery.status", "success")
    except Exception:
        r_pq = pq("<html><head></head></html>")
        span.set_attribute("unfurler.pyquery.status", "failure")

    span.add_event("pulling the data in the <head> tag")
    r_head = r_pq('head')

    span.add_event("trying to detect the favicon url")
